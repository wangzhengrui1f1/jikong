...
//        }
    ...
    private void onsuccessJsonStringMethod2(final String jsonValue, final FuncJsonString callBack) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                if (callBack != null) {
     ...
     * @param callback
     */
    public void syscFuncJsonStringByURL(String url, final FuncJsonString callback) {
        final Request request = new Request.Builder().url(url).build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
     ...
     * @param callback
     */
    public void syscFuncJsonStringByURLBean(String url, final FuncJsonString callback) {
        final Request request = new Request.Builder().url(url).build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
    ...
    //todo post请求带参数
    public void SendDatafForClicent(String url, Map<String, String> params, final FuncJsonString callback) {
        //表单对象，包含input开始的操作
        FormBody.Builder from = new FormBody.Builder();
        //键值对不为空，他的值也不为空
        if (params != null && !params.isEmpty()) {
